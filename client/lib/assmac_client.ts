import {
  Address,
  OrderPromoCodeAndPricingResponse,
  AssmacResponse,
  BaseData,
  CreateOrderResponse,
  Customer,
...
export class AssmacAPI {
  ...
  public async product(): Promise<AssmacResponse | undefined> {
    try {
      ...
      if (!resp.ok) {
        throw new Error(
          `HTTP error! status: ${resp.status} ${resp.statusText} ${await resp.text()}`,
        );
      }
  ...
  public async createOrder(
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `HTTP error! status: ${response.status} ${response.statusText} ${await response.text()}`,
        );
      }
  ...
  public async calculatePriceOrder(
    orderId: string,
    pickupPoint: boolean,
  ): Promise<PriceOrderResponse | undefined> {
    try {
      const response = await fetch(
        `${this.baseUrl}/api/orders/${orderId}/price_calculator?pickup_point=${pickupPoint}`,
      );
      if (!response.ok) {
        throw new Error(
          `HTTP error! status: ${response.status} ${response.statusText} ${await response.text()}`,
  ...
  public async applyPromoCodeToOrder(
    orderId: string,
    promoCode: string,
  ): Promise<OrderPromoCodeAndPricingResponse | { error: string }> {
    try {
      const response = await fetch(
        `${this.baseUrl}/api/orders/${orderId}/promo_code/${promoCode}`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
  ...
  public async removePromoCodeFromOrder(
    orderId: string,
  ): Promise<OrderPromoCodeAndPricingResponse | { error: string }> {
    try {
      const response = await fetch(
        `${this.baseUrl}/api/orders/${orderId}/promo_code`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
  ...
  public async getReview(token: string): Promise<
    | {
        order_id: string;
        name: string;
        stars: number | null;
  ...
  > {
    try {
      const response = await fetch(`${this.baseUrl}/api/reviews/${token}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          Accept: "application/json",
      ...
      pino().error(error, "Error fetching review");
      return { error: (error as Error).message };
    }
  }
  ...
  public async submitReview(
    ...
    | { message: string; review: { stars: number; content: string } }
    | { error: string; details?: string[] }
  > {
    try {
      const reviewData: { stars: number; content: string; name?: string } = {
        stars,
        content,
      };
      if (name) {
      ...
      pino().error(error, "Error submitting review");
      return { error: (error as Error).message };
    }
  }
}
