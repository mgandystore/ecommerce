...
export function StripePayment({
  ...
  // Clear error when form state changes (user is fixing their input)
  useEffect(() => {
    if (error) {
      setError(undefined);
    }
  }, [state.customer, state.shipping]);
  ...
  const validateCustomerAndShipping = () => {
    ...
    if (
      Object.keys(validatedOrderState.errors.customer).length > 0 ||
      ...
      setError(
        "Un ou plusieurs champs sont invalides. Veuillez vérifier vos informations.",
      );
      return false;
    }
  ...
  const processOrder = async () => {
    ...
    if ("error" in mayOrder) {
      setError(
        "Une erreur est survenue lors du traitement de la commande. Veuillez réessayer.",
      );
      return false;
    }
    return true;
