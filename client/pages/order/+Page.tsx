...
export default function Page() {
  const [deliveryMethod, setDeliveryMethod] = useState<"home" | "pickup_point">(
    "home",
  );
  ...
  useEffect(() => {
    new AssmacAPI()
      .calculatePriceOrder(data.order.id, orderState.shipping.pickup_point)
      .then((result) => {
        if (result) {
          setPrice(result);
        }
  ...
  const applyPromo = async () => {
    ...
    setPromoError(null);
    const result = await new AssmacAPI().applyPromoCodeToOrder(
      data.order.id,
      promoCode.trim(),
    );
  ...
  const removePromo = async () => {
    setApplyingPromo(true);
    setPromoError(null);
    const result = await new AssmacAPI().removePromoCodeFromOrder(
      data.order.id,
    );
      ...
      <div className="md:col-span-7 bg-white">
        <div className="p-6 md:p-12 max-w-3xl ml-auto">
          ...
          <div className="border rounded-md p-6 mb-6">
            ...
            {/* Livraison en point relais temporairement désactivée
						    La Poste a décidé de ne plus supporter ce service pour notre plateforme.
						    La fonctionnalité reste dans le code pour une éventuelle réactivation future. */}
                ...
                <div className="flex" key={item.product_variant_id}>
                  ...
                  <div className="ml-4 flex-1">
                    <p className="font-medium">
                      {item.product_name} {item.variant_human_format}
                    </p>
            ...
            <div>
              <div className="flex mb-4">
                <div className="flex-1 mr-2">
                  ...
                  />
                  {promoError && (
                    <p className="text-red-500 text-sm py-1">{promoError}</p>
                  )}
                </div>
            ...
            <div className="space-y-3 border-t pt-4">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  {price.shipping > 0 ? price.shipping / 100 : 0} €
                </span>
              </div>
